...
//                             }
    ...
    //按条件查询出总数量
    public function total_record($condition = NULL){
       // var_dump($condition);exit;
        if(!$condition){
            return $this->count();
        }else{
    ...
    //改变用户的状态
    public function change($uid,$status){
        $condition['id'] = $uid;
       // $condition['status'] = $status;
    ...
    public function saveUser($user = null){
        if(!isset($user['id']) || !isset($user['rid'])){
            return false;
        }
        ...
        return $res && $role_user->where($condition)->save($data);
